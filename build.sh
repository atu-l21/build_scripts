...
function clone_or_checkout() {
    ...
    if [[ -d "$dir" ]];then
        ...
        git clone https://github.com/atu-l21/"$repo" "$dir" -b "$localManifestBranch"
    fi
}

function init_local_manifest() {
    clone_or_checkout .repo/local_manifests manifest
}

function init_patches() {
    if [[ -n "$treble_generate" ]]; then
        clone_or_checkout patches patches
